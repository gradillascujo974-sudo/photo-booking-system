#!/usr/bin/env python3
"""光影约拍 - 一键部署脚本"""
import os, sys, subprocess, paramiko, time
...
# 2. Package
print("\n[2/4] 打包部署文件...")
deploy_file = os.path.join(PROJECT_DIR, "deploy.tar.gz")
if os.path.exists(deploy_file):
    os.remove(deploy_file)

tar_cmd = 'tar -czf "{}" dist/ backend/target/photo-booking-backend-1.0.0.jar'.format(deploy_file)
run_cmd(tar_cmd, "打包")
size_mb = os.path.getsize(deploy_file) / 1024 / 1024
print(f"打包完成: {size_mb:.1f} MB")

# 3. Upload
print(f"\n[3/4] 上传到服务器 {SERVER_IP}...")
sftp_upload(SERVER_IP, SERVER_USER, SERVER_PASS,
            deploy_file, "/root/deploy.tar.gz", "上传")
